...
| Replies as `textContent` | no `innerHTML`, no `eval`, so a model can't inject markup into its own reply |
...
**On the origin check specifically:** it only works because a browser is honest about the `Origin` header. Something that isn't a browser is not. `curl -H "Origin: https://whyjose.dev" ...` gets through fine. Tried it against the live endpoint myself to be sure. So it stops another site embedding a script that spends my OpenRouter credits from someone's browser. It does not stop anyone hitting the endpoint directly. The rate limit is what actually caps that.
